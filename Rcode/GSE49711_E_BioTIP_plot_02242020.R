# This code present BioTIP-identified CTSs from data of GSE49711, Event-occuring sampels.
...
######  BioTIP score, shulffing genes ##############
  C= 1000
  n <- length(CTS)
  BioTIP_score <- getIc(count_matrix, samplesL, as.character(CTS), fun="BioTIP", shrink=TRUE)
  BioTIP_score
...
#### BioTIP score,  shulffing labelling
 ...
 # load(file="F:/projects/BioTIP/result/GSE49711/E_simuBioTIP.RData")  
  ...
  dev.copy2pdf(file=paste0("ROutputFigs/GSE49711/Dataset1_E_BioTIP_vsSimulation_",length(CTS),"CTS_4publication.pdf"))
  ...
  C= 1000
  n <- length(CTS)
  Ic_score <- getIc(count_matrix, samplesL, as.character(CTS), fun="cor")
  Ic_score
 # LR-E     ILR-E     HIR-E      HR-E 
...
#  ns <- length(samplesL[[x]])  # of fixed number of cells
  ...
  plot_Ic_Simulation(Ic_score, simuIc_g, las = 2, ylab="Ic",ylim=c(0.1, 1),
                     main=paste(length(CTS),names(samplesL)[x], "genes", "\n","vs. ",C, "gene-permutations"),
                     fun="boxplot", which2point= x)
  plot_SS_Simulation(Ic_score, simuIc_g, 
                     main = paste("Delta Ic",n,"genes"), 
                     ylab=NULL)
  plot_Ic_Simulation(Ic_score, simuIc_s, las = 2, ylab="Ic", ylim=c(0.1, 1),
                     main=paste(length(CTS), names(samplesL)[x],"genes", "\n","vs. ",C, "sample-permutations"),
                     fun="boxplot", #fun="matplot", 
                     which2point= x)
  ...
  dev.copy2pdf(file=paste0("ROutputFigs/GSE49711/Dataset1_E_Ic_vsSimulation_",length(CTS),"CTS4publication.pdf"))
